...
char *vertfile[PROGRAM_NUM] = {"shaders/explode.vert", "shaders/explode.vert" , "shaders/explode_point.vert" };
char *fragfile[PROGRAM_NUM] = {"shaders/explode.frag", "shaders/explode.frag" , "shaders/explode_point.frag" };
...
const float startFadeSpreadPercent = 0.1;
	...
	// you may need to do something here(pass uniform variable(s) to shader and render the model)
		...
		glUseProgram(program[mode]);
			...
			if (mode == 0) {							// normal explode effect
				...
				loc = glGetUniformLocation(program[mode], "startFadePercent");
				glUniform1f(loc, startFadeSpreadPercent);
				loc = glGetUniformLocation(program[mode], "totalFadePercent");
				glUniform1f(loc, totalFadePercent);
			}
